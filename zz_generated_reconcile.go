...
// JobCreator defines an interface to create/update Jobs
type JobCreator = func(existing *batchv1.Job) (*batchv1.Job, error)

// NamedJobCreatorGetter returns the name of the resource and the corresponding creator function
type NamedJobCreatorGetter = func() (name string, create JobCreator)

// JobObjectWrapper adds a wrapper so the JobCreator matches ObjectCreator.
// This is needed as Go does not support function interface matching.
func JobObjectWrapper(create JobCreator) ObjectCreator {
	return func(existing ctrlruntimeclient.Object) (ctrlruntimeclient.Object, error) {
		if existing != nil {
...
// ReconcileJobs will create and update the Jobs coming from the passed JobCreator slice
func ReconcileJobs(ctx context.Context, namedGetters []NamedJobCreatorGetter, namespace string, client ctrlruntimeclient.Client, objectModifiers ...ObjectModifier) error {
	for _, get := range namedGetters {
		name, create := get()
		create = DefaultJob(create)
		...
		if err := EnsureNamedObject(ctx, types.NamespacedName{Namespace: namespace, Name: name}, createObject, client, &batchv1.Job{}, false); err != nil {
			return fmt.Errorf("failed to ensure Job %s/%s: %v", namespace, name, err)
		}
	}
